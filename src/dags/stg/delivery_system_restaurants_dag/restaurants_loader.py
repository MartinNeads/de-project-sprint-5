...
class restaurantsOriginRepository:
    ...
    def list_restaurants(self, restaurant_threshold: int, limit: int) -> List[restaurantObj]:
        ...
        response = requests.get('https://d5d04q7d963eapoepsqr.apigw.yandexcloud.net/restaurants', params=params, headers=headers)
        objs=response.json()
        keys=[0,1]
        print(objs)
        #objs=json2str(objs)
...
class restaurantLoader:
    WF_KEY = "example_restaurants_origin_to_stg_workflow"
